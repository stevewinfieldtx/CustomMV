...
def poll_music_status(task_id, original_request):
    ...
    if state == "SUCCESS" and info_list:
        entry     = info_list[0]
        audio_url = entry.get('audio_url') or entry.get('audioUrl')
        if not audio_url:
            raise Exception(f"No audio_url in record-info: {body}")
        ...
        raise Exception(f"Music generation failed (status={state}): {body}")
